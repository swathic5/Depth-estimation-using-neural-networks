...
function error = demo_DCNF_FCSP_depths_prediction(env, image_index, varargin)

run('/home/suriya/Swathi/fayao-dcnf-fcsp-f66628a4a991/libs/vlfeat-0.9.18/toolbox/vl_setup');

dir_matConvNet='/home/suriya/Swathi/fayao-dcnf-fcsp-f66628a4a991/libs/matconvnet_20141015/matlab/';
dir_matConvNet_mex='/home/suriya/Swathi/fayao-dcnf-fcsp-f66628a4a991/libs/matconvnet_20141015/matlab/mex';
addpath(genpath(dir_matConvNet));
addpath(genpath(dir_matConvNet_mex));
...
% some outdoor image examples:
    ...
    fprintf("Choose environment first");
end
...
depths_file = strcat(img_dir, 'actual_depths_', num2str(img_idx), '.mat')
actual_depths = load(depths_file)
    ...
    if strcmpi(img_type, 'indoor')
        actual_depths_inpaint = do_inpainting(actual_depths.A, img_data, sp_info);
        error = immse(actual_depths.A, depths_pred) * 100;
    end
    if strcmpi(img_type, 'outdoor')
        actual_depths_inpaint = do_inpainting(actual_depths.depths_new, img_data, sp_info);
        error = immse(actual_depths.depths_new, double(depths_pred));
    end
    ...
    do_save_prediction( depths_inpaint, opts_eval, actual_depths_inpaint);
